...
OneDrive 'Bloomstone quartz/<Colour>/'. Writes bloom-porc-harvest.json."""
...
IDX_CACHE = os.path.join(SCRATCH, "bloom-porcelain.html")
if os.path.exists(IDX_CACHE) and os.path.getsize(IDX_CACHE) > 100000:
    idx = open(IDX_CACHE, encoding="utf-8").read()
else:
    idx = get("https://www.bloomstoneslondon.com/porcelain-slabs").decode("utf-8", "replace")
    open(IDX_CACHE, "w", encoding="utf-8").write(idx)
q_pages = sorted(set(re.findall(r'https://www\.bloomstoneslondon\.com/porcelaine-slabs/[a-z0-9\-]+', idx)))
k_pages = []
...
json.dump(manifest, open(os.path.join(SCRATCH, "bloom-porc-harvest.json"), "w", encoding="utf-8"),
          indent=1, ensure_ascii=False)
print("WROTE bloom-porc-harvest.json:", len(manifest), "colours")
