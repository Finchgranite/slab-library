"""Reconcile bloom-porc-harvest.json with slab-library (Bloomstones Quartz) + price book.
...
DEST_ROOT = r"C:\Users\thefi\OneDrive - Finch's Stone & marble Ltd\Brands -Slabs -Kitchens-Website\3. CERAMIC- PORCELAIN\Bloomstone"

manifest = json.load(open(os.path.join(SCRATCH, "bloom-porc-harvest.json"), encoding="utf-8"))
lib = json.load(open(os.path.join(LIB, "slabs.json"), encoding="utf-8"))
entries = [r for r in lib["slabs"] if r.get("supplier") == "Bloomstones" and r.get("material") == "Porcelain"]

pb = {}
for r in csv.DictReader(open(CSV, encoding="utf-8-sig")):
    if r["Supplier"] != "Bloomstones" or r["Material"] not in ("Porcelain", "Ceramic", "Sintered Stone"):
        continue
    pb.setdefault(r["Colour"], {})[int(r["Thickness (mm)"])] = \
        f"{int(float(r['Slab Length (mm)']))}x{int(float(r['Slab Width (mm)']))}"

DROP = {"leathered", "honed", "printed", "polished", "matt", "and", "3d", "full", "vein", "fullvein3d", "fullbody3d", "natural", "select2"}
def toks(s):
    out = set()
...
for m in manifest:
    ...
    for f in m["files"]:
        ...
        good = (1.85 <= ar <= 2.15) or (ar > 0 and 1.85 <= 1 / ar <= 2.15)
        cands.append((f["kind"] == "porcelain", vk, good, w, f["file"], ar))
